...
内置 24 个组件之外，agent 可以用 **HTML 模板定义全新组件**，存到本地组件库，之后用 `:::名字` 调用。
...
- **删除**：`delete_component`，编辑器面板里也可以直接点 × 删除。
- **编辑器**：编辑器侧栏有「**组件库**」页——左侧列出全部组件（内置 + 自定义），
  右侧用**当前主题**渲染真实效果，可一键「插入到正文」或「复制示例」，并显示该组件的可定制部位。
  自定义组件也会出现在「富组件」面板的「我的组件」分组里，
  点一下即可插入（参数默认值会自动预填）；面板每次打开都从组件库实时读取，
  agent 新建的组件刷新即见。
